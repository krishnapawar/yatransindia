...
<form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <div id="profile_picture_preview" class="mt-2">
            <img id="profile_picture_preview_img" src="{{asset($user->profile_picture??'https://via.placeholder.com/150')}}" alt="Profile Preview" class="img-fluid rounded-circle">
        </div>
    ...
    <div class="row">
        ...
        <div class="col-md-6 form-group">
            ...
            <input type="text" name="phone" class="form-control numeric-input" value="{{$user->phone}}" pattern="\d+" required>
        </div>
        <div class="col-md-6 form-group">
    ...
    <div id="addresses-container">
        @foreach($user->addresses as $index => $address)
            <div class="address-group">
                ...
                <div class="row">
                    <div class="col-md-6 form-group">
                        <label for="addresses[{{$index}}][country]">Country</label>
                        <input type="text" name="addresses[{{$index}}][country]" value="{{$address->country}}" class="form-control" required>
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="addresses[{{$index}}][city]">City</label>
                        <input type="text" name="addresses[{{$index}}][city]" value="{{$address->city}}" class="form-control" required>
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="addresses[{{$index}}][state]">State</label>
                        <input type="text" name="addresses[{{$index}}][state]" value="{{$address->state}}" class="form-control" required>
                    </div>
                    <div class="col-md-6 form-group">
                        <label for="addresses[{{$index}}][zip_code]">ZIP Code</label>
                        <input type="text" name="addresses[{{$index}}][zip_code]" value="{{$address->zip_code}}" class="form-control numeric-input" pattern="\d+" required>
                    </div>
                    <div class="col-md-12 form-group">
                        <label for="addresses[{{$index}}][address_line]">Address Line</label>
                        <input type="text" name="addresses[{{$index}}][address_line]" value="{{$address->address_line}}" class="form-control" required>
                    </div>
                </div>
